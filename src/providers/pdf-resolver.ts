...
/** All known Sci-Hub mirrors (from sci-hub.pub), officials first */
...
export async function resolvePdfBuffer(identifier: string): Promise<ResolveResult | null> {
    ...
    // Source 9: Google Scholar (works with title or any text query)
...
export function buildResolutionError(source: string, doi: string | null): string {
    ...
    if (!doi) {
        suggestions.push('• No se pudo detectar un DOI. Proporciona el DOI directamente (ej: "10.1016/j.jclepro.2019.03.031")');
    }

    if (isSandboxPath(source)) {
        suggestions.push('• El path proporcionado es de un sandbox (Claude/sistema) que este servidor no puede acceder');
        suggestions.push('• En su lugar, proporciona: un DOI, una URL directa al PDF, o un link de repositorio');
    }

    suggestions.push('• Fuentes intentadas: Sci-Hub (8 mirrors), Unpaywall, CORE, Semantic Scholar, PMC, DOI.org, OA.mg, WeLib, Google Scholar');
    suggestions.push('• Si el paper es muy reciente o está detrás de paywall estricto, intenta con una URL directa al PDF');
