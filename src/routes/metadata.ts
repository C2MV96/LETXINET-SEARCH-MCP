 ...
 * POST /api/metadata/fetch  — Fetch single paper metadata
 */
...
import { extractMetadataFromHtml, fetchDSpace7Metadata, extractAliciaMetadata } from '../scraping/metadata-extractor';
...
async function enrichOnePaper(paper: PaperResult): Promise<PaperResult> {
    ...
    if (fieldsRecovered.length > 0) {
        ...
        enriched.enrichedFields = fieldsRecovered;
    }
