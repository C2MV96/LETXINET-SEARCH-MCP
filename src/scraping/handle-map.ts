...
/**
 * Comprehensive mapping of Handle.net prefixes to Peruvian university repositories
 */
export const PERU_HANDLE_MAP: Record<string, RepositoryConfig> = {
    ...
    '20.500.12996': { base: 'https://repositorio.lamolina.edu.pe/handle', name: 'Universidad Nacional Agraria La Molina', isDSpace7: true, apiBase: 'https://repositorio.lamolina.edu.pe/server/api' },
...
/**
 * Resolve handle URL to direct repository URL
 */
export function resolveHandleUrl(url: string): { url: string; config: RepositoryConfig | null } {
    // Handle hdl.handle.net URLs -> convert to direct repository URLs
    if (url.includes('hdl.handle.net')) {
        for (const [prefix, config] of Object.entries(PERU_HANDLE_MAP)) {
            if (url.includes(prefix)) {
                ...
                if (matches) {
                    const handleSuffix = matches[0];
                    return {
                        url: `${config.base}/${handleSuffix}`,
                        config
                    };
                }
            }
    ...
    // Also resolve repository /handle/ URLs that contain a known prefix
    ...
    // ALICIA URLs don't need handle resolution
